...
@app.route('/fuzzbot/test')
def render_test_page():
    return send_from_directory('frontend/views', 'test.html')

@app.route('/fuzzbot/about')
def render_about_page():
    return send_from_directory('frontend/views', 'about.html')

@app.route('/fuzzbot/results')
def render_results_page():
    return send_from_directory('frontend/views', 'results.html')

@app.route('/fuzzbot/assets/<path:path>')
def send_asset(path):
    return send_from_directory('frontend/assets', path)

@app.route('/fuzzbot/js/<path:path>')
def send_js(path):
    return send_from_directory('frontend/js', path)

@app.route('/fuzzbot/css/<path:path>')
def send_css(path):
    return send_from_directory('frontend/css', path)

@app.route('/fuzzbot/node_modules/<path:path>')
